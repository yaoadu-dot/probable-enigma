...
def process_state_and_score(df):
    ...
    s = (1 if r['Close'] > r['EMA5'] else -1) + (1 if r['Close'] > r['EMA20'] else -1) + (1 if r['Close'] > r['EMA50'] else -1) + (1 if r['RSI'] > 50 else -1) + (1 if r['MACD_Hist'] > 0 else -1) + (1 if r['+DI'] > r['-DI'] else -1)
        ...
        for t in tickers_list:
            ...
            try:
                ...
                if h is not None:
                    ...
                    processed_nodes.append({"Asset": t, "Score": score, "Status": status, "Price": round(h.iloc[-1]['Close'], 4), "RSI": round(h.iloc[-1]['RSI'], 1), "ADX": round(h.iloc[-1]['ADX'], 1)})
            except: continue
    except: pass
...
if processed:
    ...
    c1.metric("Total Online", len(df_raw))
    c2.metric("Matches", len(df_final))
    c3.metric("Bullish Flips", len(df_raw[df_raw['Status'] == "🚀 Bullish Flip"]))
    c4.metric("Bearish Flips", len(df_raw[df_raw['Status'] == "🩸 Bearish Flip"]))
